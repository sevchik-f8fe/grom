...
export const UseWebSocket = () => {
  const { error, user, token, socketState } = useSelector(
    (state) => state.global
  );
  const dispatch = useDispatch();
  ...
  }, [error, user, token, socketState, dispatch]);
...
export const UseGeo = () => {
  ...
  const { /*error,*/ user, token, socketState, socketSendHandle, isAdmin } =
  ...
  useEffect(() => {
    const sendMessage = (msg) => {
      if (
        ...
        try {
          socketSendHandle(
            JSON.stringify({ teamName: user.teamName, coords: msg })
          );
